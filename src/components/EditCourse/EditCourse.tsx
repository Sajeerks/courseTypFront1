...
import { Box, FormHelperText, TextField, styled, FormControl, MenuItem, 
   ...
   Select, InputLabel, useTheme, Button, Typography, Paper, Divider, Chip, Stack
  } from "@mui/material"
import './EditCourse.css'
import { AppDispatch, RootState } from '../Redux/store'
import { Fragment, useEffect, useRef, useState } from 'react'
import { useParams } from 'react-router-dom'
import { addSingleLectureFrontend, deleteSingleLectureFrontend, getSingleCourseFrontend, resetAllSingleCourseStateReducer, udpateEntireCourseFrontend, updateProgessForFrontend, uploadPercentagePendingTransfer } from '../Redux/singleCourseReducer'
...
const validationSchema2 = yup.object({
  ...
  description: yup.string().required("please enter a lecture description")
...
const {loading, error, message, course,percentCompleted,pendingUploadpercentage} = useSelector((state:RootState)=>state.singleCourse)
const [courseState, setcourseState] = useState<CousseModelTypeFrontend  | null>(null )
...
const [videoFileFrominput, setvideoFileFrominput] = useState<File | null>(null)
...
if(values.description  && values.description.length> 80){
  errors.description = "long description length should be less than 80 characters"
}
// if(!values.file ){
//   errors.file = null
// }
...
const handleFileUploadForVideo = async(event:React.ChangeEvent<HTMLInputElement>) => {
  // event.persist();
  console.log("handle ile upload clicked");
...
const errors:any ={}
if(values.description  && values.description.length> 80){
  errors.description = "long description length shall less than  80 characters"
}
// if(!values.file ){
...
console.log("submitted the adding lecture");
  ...
  return (
   ...
   <div className='editCourse_main_div'>
       <Helmet><title>Edit course</title></Helmet>

    <Typography textAlign={"center"} sx={{mt:1}}>CouseID : {course?._id}</Typography>
    <Box component={"form"} 
...
<FormControl sx={{ m: 1 , minWidth:"90vw"}}>
  ...
  <Paper sx={{m:1, width:"100%", height:"100%" , display:"flex", flexDirection:"column"}} key={index}  >    
  {imageUrl && <img src={imageUrl.toString()}   
  // onClick={(e)=>ImageModler(e)(imageUrl,index)}   
  loading='lazy'
  alt='new course image' style={{maxHeight:"40vh", maxWidth:"90vw"}}/>}
{
  imageArray?.length!>=1 && <Button key={index} variant='contained' onClick={()=>{
    imageArray.length = 0
    formik.setFieldValue("file", "")
    // setimage(null)
    // setimageprev("")
    
  }} >Deselect this picture</Button>}
...
<Box maxWidth={"90vw"} maxHeight={"40vh"} overflow={"scroll"} display={"flex"}  flexDirection={"row"} >
  ...
  {courseState?.poster?.url && <img src={courseState?.poster?.url.toString()}   
  // onClick={(e)=>ImageModler(e)(imageUrl,index)}   
  
  alt='old course image' style={{maxHeight:"40vh", maxWidth:"90vw"}}/>}
  ...
  <Box>
     ...
     {(courseState && courseState.lectures.length>=1) ? courseState.lectures.map((singleLecture, _index)=>{
            return (
                  <div key={singleLecture._id} className="lectureCard_in_edit_course">
             ...
             <Stack direction={"row"} width={"100%"} >   
               <Button variant='contained' sx={{background:"red", width:"50%"}} 
               
               onClick={()=>{
                
                
                dispatch(deleteSingleLectureFrontend({courseId:courseState._id!, lectureId:singleLecture._id!}))
                // if(courseId){
                //   dispatch(getSingleCourseFrontend({courseId}))
               ...
               >Delete This Lecture</Button>        
               <Button variant='contained' sx={{ width:"50%"}}>Edit This Lecture</Button>        
               </Stack>    

               <Button fullWidth variant='contained' sx={{background:"green",}} onClick={()=>setaddnewLecture(!addnewLecture)}>Add new lecture</Button>
                     </div>
            )
        }  ):( <Box> 
          ...
          <Button variant='contained' onClick={()=>setaddnewLecture(!addnewLecture)}>Add new lecture</Button>
          </Box> 
...
>
  <Box sx={ styleForModal} component={"form"}  onSubmit={formik2.handleSubmit}>
  ...
  <TextField
     ...
     helperText={formik2.touched.title && formik2.errors.title || "enter lecture title"}
   />
...
<TextField
     ...
     helperText={formik2.touched.description && formik2.errors.description || "enter lecture description"}
   />
...
<Button component="label" variant="contained" startIcon={<CloudUploadIcon />}
  ...
  >
      ...
      <VisuallyHiddenInput type="file"    multiple   accept="video/*" onChange={handleFileUploadForVideo}/>
    </Button>
     
{videoPrev && <video className='video_Preveiw_in_modal' src={videoPrev}
...
/>}
{formik2.errors.file && <Typography color={themer.palette.error.main} variant='h5'>{formik2.errors.file}</Typography>}

{!!percentCompleted && <Typography> Upload Percentage completed {percentCompleted}% </Typography>}

      <Button sx={{mt:1}} fullWidth   type='submit' variant='contained'>Submit Lecture</Button>
  </Box>
</Modal>
